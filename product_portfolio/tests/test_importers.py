...
class ProductRelationImporterTestCase(TestCase):
    ...
    def test_productcomponent_import_mandatory_columns(self):
        ...
        importer = ProductComponentImporter(
        ...
        self.assertEqual(ProductComponent.objects.latest(
            "id"), importer.results["added"][0])
    ...
    def test_productcomponent_import_product_syntax_issues(self):
        ...
        importer = ProductComponentImporter(
            self.super_user, formset_data=formset_data)
        expected = [
            {"product": ['Invalid format. Expected format: "<name>:<version>".']}]
    ...
    def test_productcomponent_import_product_matching_proper(self):
        ...
        self.p1.version = "1.0"
        self.p1.save()
        formset_data["form-0-product"] = "{}:{}".format(
            self.p1.name, self.p1.version)
        importer = ProductComponentImporter(
            self.super_user, formset_data=formset_data)
        self.assertTrue(importer.formset.is_valid())
    ...
    def test_productcomponent_import_product_column_secured(self):
        formset_data = self.component_formset_data.copy()
        importer = ProductComponentImporter(
            self.admin_user, formset_data=formset_data)
        expected = [{"product": ["Could not find the product."]}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_productcomponent_import_duplicated_component_row_error(self):
        ...
        formset_data["form-TOTAL_FORMS"] = "2"
        formset_data["form-1-product"] = formset_data["form-0-product"]
        formset_data["form-0-component"] = "{}:{}".format(
            self.c1.name, self.c1.version)
        formset_data["form-1-component"] = "{}:{}".format(
            self.c1.name, self.c1.version)
        importer = ProductComponentImporter(
            self.super_user, formset_data=formset_data)
    ...
    def test_productcomponent_import_license_expression(self):
        ...
        self.assertEqual(pc.component.license_expression,
        ...
        expected = [
            {"license_expression": [
                "Unknown license key(s): wrong<br>Available licenses: l1, l2"]}
        ]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_productcomponent_import_save_all(self):
        formset_data = self.component_formset_data.copy()
        review_status = ProductRelationStatus.objects.create(
            label="A", dataspace=self.dataspace)
    ...
    def test_productcomponent_import_license_expression_from_choice(self):
        ...
        expected = [
            {"license_expression": [
                "Unknown license key(s): l2<br>Available licenses: l1"]}
        ]
        self.assertEqual(expected, importer.formset.errors)
        ...
        self.assertEqual(self.license2.key,
                         productcomponent.license_expression)
    ...
    def test_productpackage_import_mandatory_columns(self):
        ...
        importer = ProductPackageImporter(
            self.super_user, formset_data=formset_data)
        expected_errors = [
            {
    ...
    def test_productpackage_import_non_existing_package(self):
        ...
        self.assertFalse(importer.formset.is_valid())
        expected = [
            {"package": ["The following package does not exists: non_existing"]}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_productpackage_import_duplicated_package_multiple_objects_returned(self):
        ...
        expected_errors = [
            {"package": ['Multiple packages with the same "package.zip" filename.']}]
        self.assertEqual(expected_errors, importer.formset.errors)
...
class CodebaseResourceImporterTestCase(TestCase):
    ...
    def test_codebaseresource_import_mandatory_columns(self):
        ...
        importer = CodebaseResourceImporter(
            self.super_user, formset_data=formset_data)
        self.assertTrue(importer.formset.is_valid())
        importer.save_all()
        self.assertEqual(1, len(importer.results["added"]))
        self.assertEqual(CodebaseResource.objects.latest(
            "id"), importer.results["added"][0])
    ...
    def test_codebaseresource_import_is_deployment_path_field(self):
        ...
        for idx, value in enumerate(
            ...
            self.assertTrue(importer.formset.is_valid())
            importer.save_all()
            self.assertTrue(CodebaseResource.objects.latest(
                "id").is_deployment_path)
        ...
        for idx, value in enumerate(["False", "false", "0", "F", "f", "N", "n", "No", "no"]):
            ...
            self.assertTrue(importer.formset.is_valid())
            importer.save_all()
            self.assertFalse(CodebaseResource.objects.latest(
                "id").is_deployment_path)
    ...
    def test_codebaseresource_import_additional_details_field(self):
        ...
        importer.save_all()
        expected = {"key": "value"}
        self.assertEqual(expected, CodebaseResource.objects.latest(
            "id").additional_details)
    ...
    def test_codebaseresource_import_product_component(self):
        ...
        expected = [
            {"product_component": [
                f'The component "log4j:" is not available on {self.product1}']}
        ]
        self.assertEqual(expected, importer.formset.errors)
        ...
        importer.save_all()
        self.assertTrue(importer.formset.is_valid())
        self.assertEqual(product_component, CodebaseResource.objects.latest(
    ...
    def test_codebaseresource_import_product_package(self):
        ...
        self.assertTrue(importer.formset.is_valid())
        self.assertEqual(
            product_package, CodebaseResource.objects.latest("id").product_package)
    ...
    def test_codebaseresource_import_codebase_resource_usage(self):
        ...
        importer.save_all()
        codebase_resource0 = CodebaseResource.objects.get(
            path=formset_data["form-0-path"])
        self.assertEqual(
            ["/path_in_db/",
                "/path_in_input/"], list(codebase_resource0.deployed_to_paths)
        )
        self.assertEqual([], list(codebase_resource0.deployed_from_paths))
        codebase_resource1 = CodebaseResource.objects.get(
            path=formset_data["form-1-path"])
        self.assertEqual([], list(codebase_resource1.deployed_to_paths))
        self.assertEqual(
            ["/path/"], list(codebase_resource1.deployed_from_paths))
    ...
    def test_codebaseresource_import_proper_results_view(self):
        ...
        self.assertContains(
            response, "<strong>1 Added Codebase Resources:</strong>")
...
class ProductImportFromScanTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.super_user = create_superuser("nexb_user", self.dataspace)
        self.product1 = Product.objects.create(
            name="P1", version="1.0", dataspace=self.dataspace)
    ...
    def test_product_portfolio_product_import_from_scan_proper(self):
        ...
        importer = ImportFromScan(
            self.product1, self.super_user, scan_input_location)
        warnings, created_counts = importer.save()
        self.assertEqual([], warnings)
        self.assertEqual(
            {"Packages": 1, "Product Packages": 1,
                "Codebase Resources": 3}, created_counts
        )
        ...
        self.assertEqual(
            self.super_user, self.product1.productpackages.last().created_by)
        self.assertEqual(
            self.super_user, self.product1.codebaseresources.last().created_by)

        resource = self.product1.codebaseresources.get(
    ...
    def test_product_portfolio_product_import_from_scan_scanpipe_results(self):
        scan_input_location = self.testfiles_path / \
        ...
        resource = self.product1.codebaseresources.get(
            path="package.tar.xz-extract/package/src1")
        expected_details = {
            "md5": "2c83032298cf2cc80238d47dcd8260f0",
            "sha1": "dab23f227b065e0dc81b043a71ef7c21a51a417c",
            "size": 56,
    ...
    def test_product_portfolio_product_import_from_scan_duplicated_purl_are_imported_once(self):
        ...
        self.assertEqual(
            "Python 2 and 3 compatibility utilities", package.description)
        self.assertEqual("mit", package.license_expression)
        self.assertEqual("https://github.com/benjaminp/six",
                         package.homepage_url)

        codebase_resources = self.product1.codebaseresources.values_list(
            "path", flat=True)
    ...
    def test_product_portfolio_product_import_from_scan_input_data_validation_errors(self):
        ...
        self.assertEqual(
            {"Packages": 1, "Product Packages": 1}, created_counts)
    ...
    def test_product_portfolio_product_import_from_scan_view_errors(self):
        ...
        with scan_input_location.open() as f:
            response = self.client.post(
                import_scan_url, {"upload_file": f}, follow=True)
    ...
    def test_product_portfolio_import_packages_from_scancodeio_importer(
        ...
        self.assertEqual(created_package.package_url,
                         created_package_package_url)
        ...
        self.assertEqual("pkg:pypi/aboutcode-toolkit@10",
                         created_dependency.declared_dependency)
        self.assertEqual("aboutcode-toolkit==10",
                         created_dependency.extracted_requirement)
        ...
        self.assertEqual([purl], [str(package)
                         for package in existing["package"]])
        self.assertEqual([dependency_uid], [str(package)
                         for package in existing["dependency"]])
        self.assertEqual({}, errors)
