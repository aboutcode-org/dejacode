...
class ComponentImporterTestCase(MaxQueryMixin, TestCase):
    ...
    def test_exclude_ManyToManyFields_from_model_form(self):
        for field_name in [x.name for x in Component._meta.local_many_to_many]:
            self.assertTrue(
                field_name in ComponentImporter.model_form._meta.exclude)
    ...
    def test_component_import_cp1252_encoded_file_works(self):
        file = os.path.join(TESTFILES_LOCATION,
    ...
    def test_component_import_duplicated_column_header(self):
        ...
        importer = ComponentImporter(self.super_user, file)
        self.assertTrue(
            'Column "name" is listed more than once.' in importer.fatal_errors)
    ...
    def test_component_import_save_all(self):
        ...
        importer = ComponentImporter(
            self.super_user, formset_data=formset_data)
        importer.save_all()
        self.assertEqual([self.component_with_version],
                         importer.results["unmodified"])
        added_component = importer.results["added"][0]
        self.assertTrue(added_component)

        self.assertFalse(History.objects.get_for_object(
            added_component).exists())
    ...
    def test_component_import_clean_version_warnings(self):
        ...
        self.assertEqual(
            {"version": ["Version will been cleaned to 1.1"]
             }, importer.formset.forms[1].warnings
        )
        self.assertEqual(
            {"version": ["Version will been cleaned to 1.6"]
             }, importer.formset.forms[2].warnings
        )
    ...
    def test_component_import_multiple_duplicate_rows(self):
        ...
        self.assertTrue(
            "One of the row is a duplicate." in importer.formset.non_form_errors())
        ...
        expected = {"version": ["This row is a duplicate."], "name": [
    ...
    def test_component_import_strip_input_values(self):
        ...
        self.assertFalse(importer.fatal_errors)
        # If id is a value, that means the Component has been matched
        self.assertEqual(self.component_with_version.id,
                         importer.formset[0].instance.id)
    ...
    def test_component_import_organization_suggestions(self):
        ...
        Owner.objects.create(name="Apache 2", alias="Alias1",
                             dataspace=self.dataspace)
        ...
        self.assertFalse(importer.formset.is_valid())
        expected = [
            {"owner": ["This Owner does not exists. Suggestion(s): Apache 2."]}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_component_import_fields_help_supported_values(self):
        ...
        self.assertContains(
            response, "<li><strong>TypeA</strong></li>", html=True)
        self.assertContains(
            response, "<li><strong>TypeB</strong></li>", html=True)
        self.assertContains(
            response, "<li><strong>S1</strong></li>", html=True)
        self.assertContains(
            response, "<li><strong>S2</strong></li>", html=True)
    ...
    def test_component_import_invalid_usage_policy(self):
        ...
        expected = [
            {"usage_policy": [
                'That choice is not one of the available choices: "ComponentPolicy"']}
        ]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_component_import_nullboolean_field_values(self):
        ...
        self.assertEqual(
            False, importer.formset.forms[0].instance.is_license_notice)
        self.assertEqual(
            None, importer.formset.forms[0].instance.is_copyright_notice)
        self.assertEqual(
            True, importer.formset.forms[0].instance.is_notice_in_codebase)
        ...
        self.assertEqual(
            False, importer.formset.forms[1].instance.is_license_notice)
        self.assertEqual(
            None, importer.formset.forms[1].instance.is_copyright_notice)
        self.assertEqual(
            True, importer.formset.forms[1].instance.is_notice_in_codebase)
        ...
        self.assertEqual(
            True, importer.formset.forms[2].instance.is_license_notice)
        self.assertEqual(
            False, importer.formset.forms[2].instance.is_copyright_notice)
        self.assertEqual(
            None, importer.formset.forms[2].instance.is_notice_in_codebase)
    ...
    def test_component_import_download_template_from_view(self):
        ...
        expected = ",".join(importer.required_fields +
                            importer.supported_fields)
    ...
    def test_importers_view_num_queries_view(self):
        ...
        with self.assertMaxQueries(6):
            self.client.get(
                reverse("admin:component_catalog_subcomponent_import"))
        ...
        with self.assertMaxQueries(10):
            self.client.get(
                reverse("admin:component_catalog_component_import"))
    ...
    def test_component_import_keywords(self):
        ...
        self.assertFalse(importer.formset.is_valid())
        expected = [
            {"keywords": ['Value must be valid JSON list: ["item1", "item2"].']}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_component_import_license_expression_short_name(self):
        ...
        mixed_expression = "{} AND {}".format(
            self.license.short_name, license2.key)
        formset_data = {
            "form-TOTAL_FORMS": "1",
            "form-INITIAL_FORMS": "0",
            "form-0-name": "Django",
        ...
        self.assertEqual("license AND apache-1.1",
                         added_component.license_expression)
    ...
    def test_component_import_protected_fields_ignored(self):
        ...
        importer.save_all()
        self.assertEqual(1, len(importer.results["added"]))
        self.assertEqual(self.component_policy,
                         importer.results["added"][0].usage_policy)
    ...
    def test_component_import_clean_validate_against_reference_data(self):
        ...
        importer = ComponentImporter(
            self.alternate_super_user, formset_data=formset_data)
        self.assertFalse(importer.formset.is_valid())

        absolute_link = self.component_no_version.get_absolute_link(
            target="_blank")
    ...
    def test_component_import_clean_primary_language(self):
        ...
        expected = {"primary_language": [
            'Language will be imported with proper case: "Python"']}
        self.assertEqual(expected, importer.formset.forms[0].warnings)
        importer.save_all()
        added_component = importer.results["added"][0]
    ...
    def test_component_import_acceptable_linkages(self):
        ...
        AcceptableLinkage.objects.create(
            label="linkage1", dataspace=self.dataspace)
        AcceptableLinkage.objects.create(
            label="linkage2", dataspace=self.dataspace)
        ...
        self.assertEqual(["linkage2", "linkage1"],
                         added_component2.acceptable_linkages)
...
class PackageImporterTestCase(TestCase):
    ...
    def test_package_import_with_invalid_component_value(self):
        ...
        self.assertFalse(importer.formset.is_valid())
        expected = [
            {"component": ['Invalid format. Expected format: "<name>:<version>".']}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_package_import_proper_with_component(self):
        ...
        self.assertEqual(
            "1bfaa4643c6775fbf394137f1533659be45441e7", added_package.sha1)
        self.assertEqual(411863, added_package.size)
        # A ComponentAssignedPackage relation was created
        self.assertEqual(self.component_with_version,
                         added_package.component_set.all()[0])
    ...
    def test_package_import_filename_validation(self):
        ...
        expected = [
            {"filename": [
                "Enter a valid filename: slash, backslash, or colon are not allowed."]}
        ]
        self.assertEqual(expected, importer.formset.errors)
        ...
        added_package = importer.results["added"][0]
        self.assertEqual(
            formset_data["form-0-filename"], added_package.filename)
...
class SubcomponentImporterTestCase(TestCase):
    ...
    def test_subcomponent_import_parent_equal_child(self):
        ...
        importer = SubcomponentImporter(self.super_user, file)
        expected = [
            {NON_FIELD_ERRORS: ["This Object cannot be his own child or parent."]}]
        self.assertEqual(expected, importer.formset.errors)
    ...
    def test_subcomponent_import_save_all(self):
        component3 = Component.objects.create(
            name="log4j", version="3.0", dataspace=self.dataspace)
        ...
        self.assertEqual(self.component_with_version.pk,
                         added_subcomponent.parent.pk)
    ...
    def test_subcomponent_import_usage_policy_help_supported_values(self):
        ...
        response = self.client.get(url)
        self.assertContains(
            response, "<li><strong>{}</strong></li>".format(
                self.subcomponent_policy)
        )
        self.assertNotContains(
            response, "<li><strong>{}</strong></li>".format(self.license_policy))
