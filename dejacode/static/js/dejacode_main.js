...
function setupSelectionCheckboxes() {
    ...
    const rowCheckboxes = document.querySelectorAll(
        "table#object-list-table tbody input[type='checkbox']"
    );
    ...
    rowCheckboxes.forEach((checkbox) => {
        checkbox.addEventListener('click', function (event) {
            if (event.shiftKey && lastChecked) {
                ...
                const currentCheckboxIndex =
                    Array.from(rowCheckboxes).indexOf(checkbox);
                const lastCheckedIndex =
                    Array.from(rowCheckboxes).indexOf(lastChecked);
...
function setupBackToTop() {
    ...
    window.addEventListener('scroll', function () {
        if (window.scrollY >= 250) {
            // Page is scrolled more than 250px
...
document.addEventListener('DOMContentLoaded', () => {
    NEXB = {};
    NEXB.client_data = JSON.parse(
        document.getElementById('client_data').textContent
    );
