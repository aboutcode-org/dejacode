...
(function($) {
    ...
    window.AutocompleteWidget = {
        init: function(field_selector, object_field_id, object_link_id, display_attribute) {
            ...
            let awesomplete = new Awesomplete(field_input.get(0) , {
                ...
                filter: function(text, input) {
                    ...
                    return RegExp(Awesomplete.$.regExpEscape(input_clean), "i").test(text_clean);
                },
            ...
            field_input.on('input', function() {
                ...
                if (input_value.length >= awesomplete.minChars) {
                    $.ajax({
                        url: api_url + `?format=json&page_size=${awesomplete.maxItems}&autocomplete=1&search=${input_value}`,
