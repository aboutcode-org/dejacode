...
(function($){
            ...
            success: function(result) {
                if (result.count == 1) {
                    ...
                    if(!related_object_data.license_expression) {
                        console.log('No licenses from ' + related_field.model_name + '.id=' + object_id + ', loading all license data.');
                        return false;
                    }
                    ...
                    let license_choices_expression = related_object_data.license_choices_expression;
                    let new_header = related_field.model_name + ' license expression: <code class="license_expression">' + related_object_data.license_expression + '</code>';
                    if (license_choices_expression && related_object_data.license_expression != license_choices_expression)
                        new_header += '&nbsp;&rArr;&nbsp;&nbsp;License choices: <code class="license_expression">' + license_choices_expression + '</code>';
                    ...
                    awesomplete_instance.list = operators.concat(related_object_license_data);
                    awesomplete_instance.evaluate();
                    awesomplete_instance.close();
                    console.log('Updated license data from ' + related_field.model_name + '.id=' + object_id + ': ' + related_object_license_data);
                }
            }
        });
    }
    ...
    function update_related_field_header(related_field, header_html) {
         ...
         let expression_header = inline_fieldset.find('div.license-expression-header');
    ...
    function handle_related_field(related_field, awesomplete_instance) {
        if (related_field.val()) {
            ...
            console.log('Loaded all license data for #' + awesomplete_instance.input.id);
        }
    }
    ...
    NEXB.setup_awesomplete_builder = function(element, related_field, max_items, license_data) {
        ...
        if (related_field && related_field.length) {
            ...
            handle_related_field(related_field, awesomplete);
            console.log('Add related', related_field.model_name, 'with API URL', related_field.api_url);
        }
    };
    ...
    function add_expression_builder_in_inlines_on_add_another(form) {
        let license_expression_fields = form.find("[id$='" + expression_field_name + "']");
