...
class DejaCodeUserRegistrationTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.hcaptcha_patch = patch.object(
            hCaptchaField, "validate", return_value=True)
    ...
    def test_user_registration_form_submit(self):
        ...
        self.assertContains(
            response, "Thank you for creating your account for DejaCode")
        ...
        self.assertTrue(
            "Your DejaCode Evaluation account is pending activation." in body)
        self.assertTrue("Username: {}".format(
            self.registration_data["username"]) in body)
        ...
        self.assertEqual(new_user.first_name,
                         self.registration_data["first_name"])
        self.assertEqual(new_user.last_name,
                         self.registration_data["last_name"])
        ...
        self.assertTrue(
            "New registration for user username username@company.com" in body)
    ...
    def test_user_registration_account_activation(self):
        ...
        self.assertEqual("[DejaCode] New User registration",
        ...
        self.assertEqual(
            "[DejaCode] Please activate your account", mail.outbox[1].subject)
    ...
    def test_user_registration_activate_wrong_key(self):
        activation_url = reverse(
            "django_registration_activate", args=["wrong_key"])
    ...
    def test_user_registration_default_groups(self):
        ...
        new_user = get_user_model().objects.get(
            username=self.registration_data["username"])
        self.assertEqual(len(REGISTRATION_DEFAULT_GROUPS),
                         new_user.groups.count())
